...
if selection == "Accueil":
    ...
    st.image("medias/logo_canape.jpeg", width=1000)
    st.write("""
...
# Page KPI
    ...
    try:
      ...
      plot2 = px.line(data_frame = age_moyen1, x = 'période', y = 'age', range_y = [0,80], markers=True, color_discrete_sequence =['#1b8585']*len(df_top5_graph))
      st.plotly_chart(plot2)

      #GRAPH 3
      st.subheader("Comparaison du nombre d'acteurs dans les films et séries")
      plot3 = go.Figure()
      plot3.add_trace(go.Scatter(x=df_final_KPI['période'], y=df_final_KPI['acteurs_films'],legendgroup='films', name = 'Acteurs films', marker_color='#61D2C7'))
      plot3.add_trace(go.Bar(x=df_final_KPI['période'],y=df_final_KPI['acteurs_films'], legendgroup='films', name= 'Films', marker_color='#196b6b'))
      plot3.add_trace(go.Scatter(x=df_final_KPI['période'], y=df_final_KPI['acteurs_series'], legendgroup='séries', name = 'Acteurs séries', marker_color='#cb96b7'))
      plot3.add_trace(go.Bar(x=df_final_KPI['période'],y=df_final_KPI['acteurs_series'], legendgroup='séries', name = 'Séries', marker_color='#613650'))
      plot3.update_layout(
            xaxis_title="Période",
